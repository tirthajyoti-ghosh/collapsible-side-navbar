...
// Inject a script tag that contains click event handler code that makes toggling work
const newScript = document.createElement("script");
const inlineScript = document.createTextNode(`
    const toggleNavbar = (action) => {
        const nav = document.getElementById('left-sidebar'); // nav tag
        const sideNavToggler = document.getElementById('sidenav-toggler'); // toggler icon div tag

        if (action === 'collapse') {
            sideNavToggler.innerHTML = '${openIcon}'; // If action is 'collapse' then change the icon to 'open' icon
            sideNavToggler.setAttribute('onclick', "toggleNavbar('open')"); // Update the argument of event handler function to 'open'
            
            nav.style.width = 0;
        } else {
            sideNavToggler.innerHTML = '${collapseIcon}'; // If action is 'open' then change the icon to 'collapse' icon
            sideNavToggler.setAttribute('onclick', "toggleNavbar('collapse')"); // Update the argument of event handler function to 'collapse'
